...
The process of hyperparameter tuning was performed via random search. Our search space is detailed in \autoref{table:hyper}. We then rounded our epoch size off to a multiple of $10$ since it didn't make a significant difference.
...
\begin{tabular}{|l|l|}
    ...
    min freq                & 0, ..., 5 \\
    sample                  & 1, \SI{1e-1}, ..., \SI{1e-5} \\
    train learning rate     & 0.1, 0.05, 0.025, 0.01, ..., 0.0001 \\
    inference learning rate & 0.1, 0.05, 0.025, 0.01, ..., 0.0001 \\
...
\begin{tabular}{|l|l|l|l|l|l|l|r|}
    \hline
    Method & Embedding size & \specialcell{Frequent word \\ sub-sampling} & \specialcell{Target word \\ location} & Window size & Error rate \\ \hline
    pvdm concat & 100 & gensim \SI{1e-3} & middle & 10 & 0.20672 \\
    pvdm concat & 100 & none             & end    & 9  & 0.20108 \\
    pvdm concat & 100 & w2v \SI{1e-3}    & middle & 10 & 0.19820 \\
    pvdm concat & 100 & w2v \SI{1e-3}    & end    & 9  & 0.15684 \\
    dbow        & 100 & w2v \SI{1e-3}    & -      & -  & \textBF{0.11744} \\
    \hline
\end{tabular}
\caption{\texttt{pv-dm} and \texttt{dbow} results for logistic regression classification. All results use a learning rate of $0.001$, a batch size of $2048$, $20$ training epochs and $10$ inference epochs. Some \texttt{gensim} modifications we experimented with are also included for comparison.}
\label{table:doc_emb}
\end{table}
...
\begin{tabular}{|l|l|l|r|}
    ...
    logreg     & -           & pvdm             & 0.19820 \\
    logreg     & -           & dbow             & \textBF{0.11744}    \\
    logreg     & -           & dbow+pvdm concat & 0.12092 \\
    neural net & 25          & dbow+pvdm concat & 0.12056 \\
    \hline
\end{tabular}
\caption{Classification results for logistic regression vs neural nets. All results use a learning rate of $0.01$, $30$ epochs, and a batch size of $2048$.}
\label{table:classification}
\end{table}

\section{Conclusion}
We have described the process in which we attempted to reproduce the paper ``Distributed Representations of Sentences and Documents''. The main difficulties we encountered included the unclear or ommitted description of the initialization of neural network weights, objective where multiple batches were involved, an ambiguous description of the model which can be interpreted in 2 ways, and the sampling process used to generate each training example. We hope that our findings can guide future paper authors in the area of deep learning to describe their models in a clearer fashion so that they are more easily reproducible.
...
It alters the frequent word subsampling formula. The probability of a word being included in the document in \texttt{word2vec} is $P(w)=\max\{(t/f(w))^{1/2}, 1\}$. The altered formula is $P(w)=\max\{(t/f(w))^{1/2} + {t/f(w)}, 1\}$. Since for most words $t/f(w)\approx 0$, the difference between the two formulas are not great. We tried the altered formula but obtained a slightly better result using the original (\autoref{table:doc_emb}).

Reduced window sampling, where instead of using $c$, we use $c'\sim\mathcal{U}\{0,c\}$. This is used in in the \texttt{pv-dm} average model and in the \texttt{dbow} model where skip-gram is used. This was not used in the \texttt{pv-dm} concat model, presumably because the model required $u_{d,t}$ to have fixed in dimension. We did not experiment with this due to it not being mentioned in either paper.

The \texttt{pv-dm} model predicts the center word instead of last word, similar to the \texttt{word2vec} skip-gram model. The window sizes on the left and right are required to be the same. We found that predicting the end word gave better results, while the word embeddings are subjectively similarly good (\autoref{table:doc_emb}).
